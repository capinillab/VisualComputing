class Boid {
  ...
  Boid(Vector inPos, boolean modo, boolean representation) {
    this.modo = modo;
    ...
    if (modo && !representation) {
      ...
      faceVertex = new FaceVertex(faceArrayList, vertexArrayList);
      this.boid = faceVertex.renderRetained();
    }
  ...
  void render() {
    ...
    //vertex(3 * sc, 0, 0);        // 3, 0, 0
    //vertex(-3 * sc, 2 * sc, 0);  //-3, 2, 0
    //vertex(-3 * sc, -2 * sc, 0); //-3,-2, 0

    //vertex(3 * sc, 0, 0);        // 3, 0, 0
    //vertex(-3 * sc, 2 * sc, 0);  //-3, 2, 0
    //vertex(-3 * sc, 0, 2 * sc);  //-3, 0, 2
  ...
  // modo true Retained, false Inmediate
